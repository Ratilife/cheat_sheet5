# ANTLR4: Изучение с нуля для начинающих
...
2. ANTLR4 генерирует код
   ↓
   На основе грамматики создает лексер и парсер на выбранном языке (Python)
...
Давайте создадим простейший парсер для арифметических выражений с нуля.
...
1. **Лексер** разбил текст `"2 + 3 * 4"` на токены: `[INT:2, PLUS:+, INT:3, MUL:*, INT:4]`

2. **Парсер** проанализировал токены и построил дерево:
   - Умножение `3 * 4` находится внутри сложения
   - Это правильно отражает приоритет операций

3. **Дерево** показывает структуру выражения в виде вложенных узлов

### Расширенный пример: просмотр токенов

Добавьте в `main.py` для просмотра токенов:
...
## 5. Что дальше? Listeners и Visitors
...
Или используйте опции парсера для указания приоритета (более продвинутый способ).
...
```python
ctx.getText()  # "2 + 3 * 4" - весь текст узла
```

**Доступ к контексту** позволяет получить дочерние элементы:
```python
ctx.expr(0)    # Первое дочернее выражение (контекст, не строка!)
ctx.INT()      # Токен INT (если он есть в этом узле)
```
...
### Ошибка 5: Неправильный порядок правил в грамматике
...
### Ошибка 6: Забыл указать язык при генерации
...
ANTLR4 — это инструмент, который делает сложное простым. Начните с простых примеров, постепенно усложняйте задачи, и вы быстро освоите этот мощный инструмент для работы с языками и структурированными данными.
