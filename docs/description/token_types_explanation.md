...
- `'PREPROC_DELETE_ANY'`-
- `'AWAIT_KEYWORD'`- Ждать
- `'WHITE_SPACE'` — пробелы, табы, переносы строк
- `'HASH'` — # (может быть частью препроцессора или комментария)
- `'BAR'` — | (логический ИЛИ или битовая операция)
- `'TILDA'` — ~ (битовая операция НЕ)
- 