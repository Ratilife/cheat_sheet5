...
class MainWindow(QMainWindow):
    def __init__(self, view_model: IButtonViewModel):
        ...
        # Флаг для отслеживания состояния панели (свернута/развернута)
        ...
        icon_collapse = self.load_icon_from_base64(collapse_icon)
        self.collapse_button.setIcon(icon_collapse)
        self.collapse_button.setFixedSize(30, 30)
        self.collapse_button.clicked.connect(self.toggle_panel)     # ?
        self.buttons_layout.insertWidget(0, self.collapse_button)        # ?
        ...
        self.buttons_layout.insertWidget(1, self.add_button)
        ...
        self.buttons_layout.insertWidget(2, self.delete_button)  # Размещаем после кнопки "Добавить"
        ...
        self.buttons_layout.insertWidget(3, self.side_panel)
        ...
        self.structure_manager.clicked.connect(self.open_structure_manager_clicked)
        self.buttons_layout.insertWidget(4, self.structure_manager)
    ...
    def update_buttons(self):
        ...
        if self.is_collapsed:
            return  # Не обновляем кнопки, если панель свернута

        # Очистка текущих кнопок (кроме кнопки "Сворачивание" "Добавить", "Удалить","Боковая панель" ,"Менеджер структуры" и "Закрыть")
        for i in reversed(range(self.buttons_layout.count())):
            ...
            if (widget != self.add_button and
                    ...
                    widget != self.structure_manager and
                    widget != self.collapse_button):
        ...
        for i, button in enumerate(buttons):
            ...
            self.buttons_layout.insertWidget(5 + i, btn)  # Вставляем кнопки после "Менеджер структуры"
    ...
    def adjust_size(self):
        ...
        if self.is_collapsed:
            return  # Не изменяем размер, если панель свернута
    ...
    def toggle_panel(self):
        ...
        # TODO 🚧 В разработке: 06.08.2025
            #🏆task: Свернуть панель до размера кнопки и обратно;
        self.is_collapsed = not self.is_collapsed

        if self.is_collapsed:
            # Сворачиваем панель - оставляем только кнопку сворачивания
            for i in reversed(range(self.buttons_layout.count())):
                widget = self.buttons_layout.itemAt(i).widget()
                if widget != self.collapse_button:
                    widget.hide()

            # Меняем иконку на "развернуть"
            expand_icon = self.load_icon_from_base64(side_panel_icon)  # Можно использовать другую иконку
            self.collapse_button.setIcon(expand_icon)

            # Устанавливаем минимальную ширину
            self.setFixedWidth(40)
        else:
            # Разворачиваем панель - показываем все кнопки
            for i in range(self.buttons_layout.count()):
                widget = self.buttons_layout.itemAt(i).widget()
                widget.show()

            # Меняем иконку на "свернуть"
            collapse_icon = self.load_icon_from_base64(side_panel_icon)
...
collapse_icon = b''
