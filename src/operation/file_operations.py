...
class FileOperations:
    """
        Класс для выполнения операций с файлами в приложении.

        Предоставляет высокоуровневый интерфейс для работы с файловой системой проекта,
        включая создание и управление структурой папок, работу с ST и MD файлами,
        а также интеграцию с системой кэширования и парсинга.

        Основные возможности:
        - Создание корневой структуры проекта на основе JSON-конфигурации
        - Управление путями и сохранение конфигурации корневой папки
        - Получение иерархии файлов из папки закладок
        - Загрузка и сохранение ST/MD файлов
        - Создание новых ST и MD файлов с базовыми шаблонами
        - Работа с кэшем: добавление новых файлов в кэш после создания
        - Манипуляции со структурой ST-файлов (создание папок и шаблонов)
        - Добавление элементов (папок/шаблонов) в существующую структуру ST-файлов

        Класс использует:
        - FileManager для низкоуровневых операций с файловой системой
        - DialogManager для отображения сообщений пользователю
        - FileParserService для парсинга файлов перед добавлением в кэш
        - Систему кэширования через get_content_cache() для оптимизации доступа к данным

        Attributes:
            file_manager (FileManager): Менеджер для работы с файловой системой
            messenger (DialogManager): Менеджер для отображения диалоговых окон и сообщений
    """
    ...
    def _create_file_path(self, expansion: str, name: str, tab_name: str):
        """Создает путь к файлу на основе расширения, имени и имени вкладки.

            Получает путь к папке закладок, проверяет его существование,
            а затем конструирует полный путь к файлу в подкаталоге с именем
            вкладки внутри папки закладок.

            Args:
                expansion (str): Расширение файла (например, 'st', 'md').
                name (str): Имя файла без расширения.
                tab_name (str): Имя вкладки, используемое как имя подкаталога.

            Returns:
                str: Полный путь к файлу или пустая строка в случае ошибки.
        """
        # TODO 🚧 В разработке: 08.10.2025
        bookmarks = get_bookmarks()
