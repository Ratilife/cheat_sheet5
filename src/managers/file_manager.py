...
class FileManager:
    ...
    def create_files_dict_with_paths(self, bookmarks_path: Path) -> dict:
        """
        Создает словарь с файлами .md и .st из указанной директории и всех поддиректорий.

        Args:
            bookmarks_path (str): Путь к корневой директории для поиска файлов

        Returns:
            dict: Словарь, где:
                  - ключи: имена директорий
                  - значения: списки полных путей к файлам с расширениями .md или .st
        """
        # TODO 🚧 В разработке: 12.08.2025
        # Инициализация пустого словаря для хранения результатов
        dict_dir_files = {}

        # Рекурсивный обход файловой системы, начиная с bookmarks_path
        ...
        for root, dirs, files in os.walk(bookmarks_path):
            ...
            for file in files:
                # Проверяем, что файл имеет одно из целевых расширений (.md или .st)
                if file.endswith((".md", ".st")):
                    # Получаем имя текущей директории (последний компонент пути)
                    ...
                    # Если директория уже есть в словаре, добавляем файл в существующий список
                    if dir_name in dict_dir_files:
                        dict_dir_files[dir_name].append(file_path)
                    # Если директории нет в словаре, создаем новую запись
                    else:
                        dict_dir_files[dir_name] = [file_path]
